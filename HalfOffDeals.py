...
def getNextFile():
    ...
    # source dir
    sourceDir = settings.get('halfoffdealsdrop')
...
def archiveFile(path):
    ...
    # archive dir
    archiveDir = settings.get('halfoffdealsarchive')
...
def getItems(path, columns):
    with open(path) as file:
        ...
        for row in reader:
            ...
            newRow['itemSKU'] = row[2].split('(')[-1][:-1] # extract sku from () in title
            ...
            if len(columns) == len(newRow):
                ...
                print("Oops, Half Off Deals item parser added a column")
                quit()
...
def getPackages(path, columns):
    ...
    while orderEnd < len(lines):
        ...
        newRow['merchantID'] = 46
        newRow['shortOrderReference'] = validate.clean(currentOrder[0][0].split('_')[-1])
        newRow['returnCompany'] = 'Half Off Deals'
        ...
        if len(columns) == len(newRow):
            ...
            print("Oops, Half Off Deals shipping allocator added a column")
            quit()
